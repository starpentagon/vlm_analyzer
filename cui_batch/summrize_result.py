...
if __name__ == "__main__":
    ...
    table.add_row(['Solved', "{:,.0f}".format(solved_count), "{:,.1f}".format(
        100.0 * solved_count / problem_count)])
    table.add_row([
        'Terminated', 
        "{:,.0f}".format(terminated_count),
        "{:,.1f}".format(100.0 * terminated_count / problem_count)
    ])

    table.add_row([
        'Disproved',
        "{:,.0f}".format(disproved_count),
        "{:,.1f}".format(100.0 * disproved_count / problem_count)
    ])
    ...
    for col in col_list:
        if col == 'Time(sec)':
            table.add_row([
                col,
                "{:,.2f}".format(result_mean[col]),
                "{:,.2f}".format(result_max[col]),
                "{:,.2f}".format(result_min[col])
            ])
        else:
            table.add_row([
                col,
                "{:,.0f}".format(result_mean[col]),
                "{:,.0f}".format(result_max[col]),
