...
if __name__ == "__main__":
    ...
    # 問題数, 解図数のチェック
    problem_count = sum(1 for row in open(
        problem_db_filename, 'rb')) - 1  # header分を除く
    result_count = sum(1 for row in open(result_filename, 'rb')) - 1

    if(problem_count != result_count):
        print 'The problem db size(%d) is not equal to \
        the result size(%d).' % (problem_count, result_count)
        quit()
    ...
    for i in range(problem_count):
        ...
        if problem_id != result_id:
            print "Problem ID(%s) is not equal to \
            Result ID(%s)" % (problem_id, result_id)
            quit()
        ...
        if result == "Solved" and is_depth_check:
            ...
            if result_depth < problem_depth:
                print "Problem ID(%s): result depth(%d) is less than \
                intended depth(%d)" % (problem_id, result_depth, problem_depth)

            if result_depth > problem_depth:
                print "Problem ID(%s): result depth(%d) is more than \
                intended depth(%d)" % (problem_id, result_depth, problem_depth)
            ...
            if intended_first_move != result_first_move:
                print "Problem ID(%s): result first move(%s) is not intended \
                one(%s)" % (problem_id, result_first_move, intended_first_move)
        ...
        if result == "Terminated" and is_depth_check:
            ...
            if result_depth >= problem_depth:
                print "Problem ID(%s): result depth(>%d) is more than intended \
                depth(%d)" % (problem_id, result_depth, problem_depth)
